...
def get_credit_card_summary(session, account_id: UUID) -> CreditCardSummary | None:
    ...
    account_row = session.execute(
        select(accounts.c.available_limit)
    ...
    total_limit = account_row.available_limit if account_row.available_limit is not None else Decimal('0')
...
def list_credit_card_transactions(session, limit: int = 50, account: str | None = None, account_type: str | None = None) -> list[CreditCardTransactionOut]:
    query = select(
        credit_card_transactions.c.id,
        credit_card_transactions.c.amount,
        credit_card_transactions.c.status,
    ...
    if account_type:
        query = query.join(accounts, credit_card_transactions.c.account == func.cast(accounts.c.id, Text))
...
def update_credit_card_transaction(session, eid: UUID, data: CreditCardTransactionUpdate) -> CreditCardTransactionOut | None:
