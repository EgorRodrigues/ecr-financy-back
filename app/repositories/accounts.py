...
def update_account(session, aid: UUID, data: AccountUpdate) -> AccountOut | None:
    ...
    new_initial = (
        data.initial_balance if data.initial_balance is not None else current.initial_balance
    )
    new_limit = (
        data.available_limit if data.available_limit is not None else current.available_limit
    )
