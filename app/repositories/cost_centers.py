...
def create_cost_center(session: Session, data: CostCenterCreate) -> CostCenterOut:
    stmt = insert(cost_centers).values(
        name=data.name,
...
def update_cost_center(session: Session, cost_center_id: UUID, data: CostCenterUpdate) -> CostCenterOut | None:
    ...
    stmt = update(cost_centers).where(
        cost_centers.c.id == cost_center_id
