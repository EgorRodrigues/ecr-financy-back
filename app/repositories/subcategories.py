...
def list_subcategories(session, category_id: UUID, limit: int = 50) -> list[SubcategoryOut]:
    rows = session.execute(
        ...
        ).where(subcategories.c.category_id == category_id).limit(limit)
    ).all()
    return [
        SubcategoryOut(
...
def delete_subcategory(session, category_id: UUID, sid: UUID) -> bool:
    session.execute(delete(subcategories).where((subcategories.c.category_id == category_id) & (subcategories.c.id == sid)))
    session.commit()
    return True
...
def move_subcategory(session, from_category_id: UUID, sid: UUID, payload: SubcategoryMove) -> SubcategoryOut | None:
    ...
    now = datetime.now(timezone.utc)
    session.execute(
        update(subcategories).where(subcategories.c.id == sid).values(category_id=payload.new_category_id, updated_at=now)
    )
    session.commit()
