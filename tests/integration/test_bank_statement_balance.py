...
def test_bank_statement_balance_persistence(client: TestClient):
    # 1. Create Account
    r_acc = client.post("/accounts/", json={"name": "Balance Test", "type": "bank", "initial_balance": 1000.00, "active": True})
    aid = r_acc.json()["id"]
    ...
    response = client.get(f"/bank-statement/?start_date={future_start}&end_date={future_end}&account_id={aid}")
