...
def test_bank_statement_basic(client: TestClient):
    ...
    # Sort order is date desc. Since both are today, order might be unstable or based on insertion
    # if time matches (but date is only date).
...
def test_bank_statement_with_filters(client: TestClient):
    ...
    # Expense last month (should be excluded if we query today only,
    # but default is 1 month window so it might be included)
